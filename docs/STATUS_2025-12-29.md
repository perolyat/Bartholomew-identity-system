...
> "Windows environment issues, not logic bugs" — MASTER_PLAN.md's "FTS5 external-content
> `upsert()` bug..." section (fixed 2026-07-20) found two of those were real logic bugs,
> reproduced identically on Linux. Treat any specific figure or claim below as unverified
> until cross-checked against `MASTER_PLAN.md`/`ROADMAP.md`.
