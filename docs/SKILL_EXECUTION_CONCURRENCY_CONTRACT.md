...
| CI, Windows full default suite (Merge Candidate, `-n auto --dist loadfile`, real Win32) | all 28 tests green in both runs that carry the second pass — run 35034729745 on `04db28d` (23:23:00–23:23:31) and run 35037740445 on `8cf3707` (00:03:30–00:04:04) — with the governance-store seed test and the eight record-serialization tests, no `database is locked` and no contract refusal (`Skill busy`, `Skill not ready`, `Re-entrant action refused`, `Skill action timed out`, `Skill action cancelled`) anywhere in either log |
| CI, Ubuntu (PR Fast, Integration, Merge Candidate 3.10 and 3.11) | green on the final head; the contract's tests run in every default-suite job |
