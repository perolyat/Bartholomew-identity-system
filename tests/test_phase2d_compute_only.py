...
class TestComputeOnly:
    ...
    async def test_embed_compute_only_does_not_persist_rows_and_returns_vectors(
        ...
        try:
            ...
            assert vec_store.count() == 0, "Vectors should not be persisted when embed_store=False"
            ...
            assert len(results) == 0, "Memory should not be retrievable via vector search"
        finally:
    ...
    @pytest.mark.asyncio
    async def test_compute_only_then_persist_after_consent(self, tmp_path, monkeypatch):
