...
async def test_migrate_schema_idempotent():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        await store.upsert_memory("fact", "key1", "Content one", "2025-01-01T12:00:00Z")
        await store.upsert_memory("fact", "key2", "Content two", "2025-01-01T12:01:00Z")
...
async def test_migrate_schema_fixes_rowid_mismatch():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Insert test memory
        await store.upsert_memory("fact", "test_key", "Test content", "2025-01-01T12:00:00Z")
        ...
        assert orphaned_before > 0, "Should have orphaned entry before migration"
...
async def test_optimize_method():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        await store.upsert_memory("fact", "key1", "Content alpha", "2025-01-01T12:00:00Z")
        await store.upsert_memory("fact", "key2", "Content beta", "2025-01-01T12:01:00Z")
        await store.upsert_memory("fact", "key3", "Content gamma", "2025-01-01T12:02:00Z")
