    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for row in dataset:
            ...
            if group_id not in seen_groups:
                ...
                queries.append({"text": row["text"], "group_id": group_id, "query_id": row["id"]})
                        ...
                        if row["id"] in memory_ids:
                            if memory_ids[row["id"]] == result.memory_id:
                                if int(row["group_id"]) == query["group_id"]:
                                    hits += 1
                                    break
                        if result.memory_id in [memory_ids.get(row["id"]) for row in dataset]:
                            break
        ...
        assert (
            hybrid_hit_rate >= best_single
        ), f"Hybrid ({hybrid_hit_rate:.2f}) should be >= best single ({best_single:.2f})"
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for row in dataset:
            ...
            if row["privacy_marker"] in ("never_store", "requires_consent"):
                ...
                assert (
                    not result.stored
                ), f"Memory with {row['privacy_marker']} should not be stored"
            elif result.stored:
        ...
        test_queries = ["password storage", "personal data consent", "privacy protection"]
            ...
            for query in test_queries:
                try:
                    ...
                    for result in results:
                        assert (
                            result.snippet not in blocked_texts
