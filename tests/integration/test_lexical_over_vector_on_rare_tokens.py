...
from tests.helpers.synthetic import (
    create_uncorrelated_embeddings,
    make_rare_token
)
...
async def test_lexical_beats_vector_on_exact_rare_tokens():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert hybrid_top1_accuracy >= vec_top1_accuracy, (
            f"Hybrid ({hybrid_top1_accuracy:.2%}) should be ≥ "
            f"vector ({vec_top1_accuracy:.2%})"
        )
...
async def test_lexical_top_k_coverage_on_rare_tokens():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        test_queries = [(corpus[i]['token'], corpus[i]['token']) 
        ...
        assert fts_topk_recall >= 0.92, (
            f"FTS top-5 recall ({fts_topk_recall:.2%}) should be ≥92%"
        )
