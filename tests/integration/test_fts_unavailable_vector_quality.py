...
def calculate_hit_rate(results_list, queries, memory_map):
    ...
    hits = 0
    for query_info, results in zip(queries, results_list):
...
async def test_vector_quality_maintained_when_fts_unavailable():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for item in corpus:
            ...
            memory_map[(item['group_id'], item['variant_idx'])] = (
                result.memory_id
            )
        ...
        queries = []
        for group_id in range(min(10, len(set(
            item['group_id'] for item in corpus
        )))):
        ...
        vec_hit_rate = calculate_hit_rate(
            vec_results_list, queries, memory_map
        )
        ...
        with patch('bartholomew.kernel.retrieval.fts5_available') as mock:
            ...
            hybrid_hit_rate = calculate_hit_rate(
                hybrid_results_list, queries, memory_map
            )
        ...
        assert hybrid_hit_rate >= 0.60, (
            f"Hybrid with FTS unavailable ({hybrid_hit_rate:.2%}) should "
            f"maintain ≥60% hit rate"
        )
        
        assert vec_hit_rate >= 0.60, (
            f"Vector baseline ({vec_hit_rate:.2%}) should be ≥60%"
        )
...
async def test_no_crash_on_fts_queries_when_unavailable():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for item in corpus:
            ...
            memory_id = memory_map[key]
            vec = create_synthetic_embeddings(
                item['group_id'], item['variant_idx'], dim=384, seed=2024
            )
            vec_store.upsert(
                memory_id=memory_id,
                vec=vec,
            ...
            for query in test_queries:
                try:
                    ...
                    assert isinstance(results, list), (
                        f"Query '{query}' should return list"
                    )
                except Exception as e:
                    pytest.fail(
                        f"Query '{query}' crashed with FTS unavailable: {e}"
                    )
