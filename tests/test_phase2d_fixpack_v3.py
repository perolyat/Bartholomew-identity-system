...
class TestSummaryFallback:
    ...
    @pytest.mark.asyncio
    async def test_embed_falls_back_when_summary_missing(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            assert (
                vec_store.count() > 0
            ), "Should have embeddings despite missing summary (fallback to content)"
        finally:
            summarization_engine._summarization_engine.should_summarize = (
                original_should  # noqa: E501
            )
...
class TestRetrievalBoost:
    ...
    def test_boost_affects_ranking(self, tmp_path):
        ...
        # Insert two memories
        mem1_id = insert_test_memory(conn, kind="normal", key="key1", value="hello world")
        mem2_id = insert_test_memory(conn, kind="boosted", key="key2", value="hello world")
        conn.close()
        ...
        try:
            ...
            assert results[0].kind == "boosted", "Boosted memory should rank first"
            assert results[0].score > results[1].score, "Boosted score should be higher"
        finally:
            memory_rules._rules_engine.evaluate = original_evaluate
...
class TestStrictModelMatching:
    ...
    def test_strict_excludes_mismatches(self, tmp_path):
        ...
        vec_store.upsert(mem1_id, vec, "summary", "local-sbert", "model-a")
        vec_store.upsert(mem2_id, vec, "summary", "openai", "model-b")
...
class TestPolicyFlags:
    ...
    def test_context_only_flag_set(self, tmp_path):
        ...
        conn = connect_test_db(db_path)
        create_minimal_memories_table(conn)
        mem_id = insert_test_memory(conn, kind="context", key="key1", value="test content")
        conn.close()
