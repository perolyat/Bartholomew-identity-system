...
class TestRRF:
    ...
    def test_rrf_k_effect(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Small k
            config_small = HybridRetrievalConfig(
                fusion_mode="rrf", rrf_k=10
            )
            ...
            # Large k
            config_large = HybridRetrievalConfig(
                fusion_mode="rrf", rrf_k=100
            )
    ...
    def test_kind_boosts(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            config = HybridRetrievalConfig(
                kind_boosts={"important": 2.0, "regular": 1.0}
            )
            retriever = HybridRetriever(db_path, config=config)
            ...
            boosted_fts, boosted_vec = retriever._apply_boosts(
                fts_scores, vec_scores, metadata, rules_data
            )
