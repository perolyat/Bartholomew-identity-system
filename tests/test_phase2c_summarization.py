...
class TestMemoryStoreIntegration:
    ...
    async def test_auto_summarize_long_transcript(self, tmp_path):
        ...
        long_value = (
            "This is a very long conversation transcript. " * 30
        )
        ...
        async with aiosqlite.connect(db_path) as db:
            cursor = await db.execute(
                "SELECT value, summary FROM memories "
                "WHERE kind=? AND key=?",
    ...
    @pytest.mark.asyncio
    async def test_summary_encrypted_with_value(
        ...
        try:
            ...
            # Decrypt and verify
            decrypted_value = new_engine.try_decrypt_if_envelope(
                stored_value
            )
            decrypted_summary = new_engine.try_decrypt_if_envelope(
                stored_summary
            )
...
class TestYAMLRulesIntegration:
    ...
    def test_yaml_rules_include_summary_mode(self):
        ...
        if os.path.exists(yaml_path):
            ...
            assert has_summary_mode, \
                "At least one rule should specify summary_mode"
