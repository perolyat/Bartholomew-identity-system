...
class TestVectorStore:
    ...
    def test_upsert_and_search(self, tmp_path):
        ...
        from tests.helpers import connect_test_db, create_minimal_memories_table, insert_test_memory
...
class TestMemoryStoreIntegration:
    ...
    @pytest.mark.asyncio
    async def test_embeddings_with_env_and_rule(self, tmp_path, monkeypatch):
...
class TestRetriever:
    ...
    def test_retriever_basic_query(self, tmp_path):
        ...
        conn = connect_test_db(db_path)
        create_minimal_memories_table(conn)
        mem_id = insert_test_memory(conn, kind="test", key="key1", value="hello world")
        conn.close()
...
class TestEmbedStoreDefaults:
    ...
    @pytest.mark.asyncio
    async def test_embed_store_defaults_to_true_when_embed_set(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            vec_store = VectorStore(db_path)
            assert vec_store.count() > 0, "embed_store should default to True when embed != 'none'"
        finally:
            memory_rules._rules_engine.evaluate = original_evaluate
            monkeypatch.delenv("BARTHO_EMBED_ENABLED", raising=False)
...
class TestForeignKeyCascade:
    ...
    @pytest.mark.asyncio
    async def test_deleting_memory_removes_embeddings(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            async with aiosqlite.connect(db_path) as db:
                ...
                await db.execute("DELETE FROM memories WHERE kind=? AND key=?", ("test", "test1"))
                await db.commit()

            # Embeddings should be cascade deleted
            assert vec_store.count() == 0, "FK cascade should delete embeddings when memory deleted"
        finally:
            memory_rules._rules_engine.evaluate = original_evaluate
            monkeypatch.delenv("BARTHO_EMBED_ENABLED", raising=False)
...
class TestLifecycleMethods:
    ...
    @pytest.mark.asyncio
    async def test_persist_embeddings_for_after_consent(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_reembed_memory_replaces_vectors(self, tmp_path, monkeypatch):
...
class TestConfigAndRules:
    ...
    def test_embeddings_yaml_exists(self):
        ...
        config_path = os.path.join("bartholomew", "config", "embeddings.yaml")
