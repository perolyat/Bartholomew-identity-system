...
def main():
    parser = argparse.ArgumentParser(
        description="Hybrid search demo with explainable scores"
    )
    parser.add_argument(
        "--query", "-q",
        required=True,
    ...
    parser.add_argument(
        "--rrf",
        action="store_true",
        help="Use RRF fusion mode (only for hybrid)"
    )
    parser.add_argument(
        "--rrf-k",
        type=int,
    ...
    parser.add_argument(
        "--db",
        help="Database path (default: from config/env)"
    )
    parser.add_argument(
        "--explain",
        action="store_true",
    ...
    if args.explain and isinstance(retriever, HybridRetriever):
        ...
        print(f"Weights: FTS={retriever.config.weight_fts:.2f}, "
    ...
    if args.show_features:
        if isinstance(retriever, HybridRetriever) and retriever.last_debug:
            ...
            if fusion_mode == "weighted":
                ...
                print(f"Weights: FTS={weights.get('fts', 0):.3f}, "
                      f"Vec={weights.get('vec', 0):.3f}")
            
            # Per-result features
            per_result = retriever.last_debug.get("per_result", [])
            if per_result:
                print(f"\nPer-Result Features (top-{len(per_result)}):")
                print("Rank   ID       BM25     Vec      "
                      "Recency  Kind     Final   ")
                print("-" * 62)
