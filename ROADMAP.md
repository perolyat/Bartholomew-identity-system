...
| **B1** — Shared SQLite connection policy | One connection/pragma/close policy; inventory and assign every remaining consumer migration | B0 | ✅ Complete 2026-07-31 | `docs/B1_IMPLEMENTATION.md` — the API bridge's duplicate `db_ctx.py` is now a re-export of the kernel module, not a second implementation; fixed a real bug the consolidation exposed (the API copy's `wal_db()` unconditionally ran a blocking `TRUNCATE` checkpoint on every liveness-API read); all 24 remaining connection-owner files from B0 inventoried and assigned to B2 (7 event-loop-blocking files) or B8 (17 remaining consumers) |
| **B2** — Event-loop isolation and database execution | Remove blocking sync SQLite calls from the event loop | B1 | ✅ Partial 2026-07-31 | `docs/B2_IMPLEMENTATION.md` — new `DedicatedDbExecutor` (generalizes `SchedulerStore`'s proven pattern); 6 of 7 B1-assigned files fully resolved (`memory_store.py`, `skill_registry.py`, `tasks.py`, `notify.py`, `calendar_draft.py`, `persona_pack.py`); `narrator.py`'s `persist_episode()` is an **open finding**, reached via `GlobalWorkspace`'s synchronous cross-cutting dispatch — deferred, not resolved, honestly recorded rather than claimed complete |
| **B3** — Governance schema and Parking Brake persistence | One durable, auditable Governance representation | B2 | ✅ Complete 2026-07-31 | `docs/B3_IMPLEMENTATION.md` — new `bartholomew/kernel/governance/` package (`parking_brake_state`/`brake_runtime`/`governance_audit` schema, `GovernanceBrakeStore` transitions), tested in isolation (26 tests), no runtime wiring. Fixed two real defects found in the live `ParkingBrake` while designing this: non-monotonic `engage()` (replaces scopes instead of unioning — can silently loosen) and no confirmed-loosening gate on `disengage()` |
| **B4** — Shared Governance runtime integration | One shared Parking Brake instance at every real live-daemon call site | B3 | ✅ Complete 2026-07-31 | `docs/B4_IMPLEMENTATION.md` — re-inventoried live sites: all 6 are read-only (`is_blocked()` only; `engage()`/`disengage()` only exist in CLI, out of scope). One shared `check_scope_blocked()` function replaces 6 independent inline constructions, still per-call-fresh (a literal long-lived cached singleton would go stale against CLI engage/disengage — considered and rejected). User confirmed staying on the legacy `system_flags` schema rather than switching live reads to B3's new schema now, to avoid a window where CLI writes and daemon reads target different tables; the schema swap happens together with B6 |
| **B5** — Startup and shutdown integrity | Reliable failure handling; clean-shutdown evidence for B1–B4's own resources, as lifecycle-terminal-state conditions (no process lock or external-admission draining yet) | B1–B4 | ✅ Complete 2026-07-31 | `docs/B5_IMPLEMENTATION.md` — found and fixed a real gap: failed-start unwind only drained `scheduler_store`, leaking B2's newer `mem`/`skill_registry`/skill executors on a startup failure after skills loaded. Added a clean-shutdown lifecycle marker (`bartholomew/kernel/lifecycle_marker.py`) with conservative (log-only) unclean-start detection, and a poisoned-instance guard refusing `start()` reuse after a failed start. "Governance write freeze and drain" explicitly recorded as not-applicable yet — B4 kept Governance on a stateless legacy path with nothing live to freeze |
