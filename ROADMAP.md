...
>
> **(1) This document's stage/band structure did not know about the September work.** Wave 3
> (PR #101), BGPR-01 (#102), FND-01 to FND-04 (#104–#107) and EXEC-01 (#108) are all merged.
> They were sequenced as bounded repair/integration packages rather than as new stages, so
> the stage gates and readiness bands below are unaffected — but a reader must not conclude
> from this document alone that the front of the queue is where it says it is.
...
>
> **(2) EXEC-02 is MERGED, and it moves no band.** Deepening the existing Executive so the
> conversational surface reaches its goal-to-plan deliberation was merged on 2026-09-18 at Taylor's
> User Approval Gate — PR #115, merge commit `25cfd90` (`START_HERE.md` §5a;
> `docs/EXEC_02_CONVERSATIONAL_EXECUTIVE_INTEGRATION.md`). **Taylor sequenced it on 2026-09-14** to
> follow a narrow Windows writer-lock/WAL reliability repair and one attended **Band 0** checkpoint;
> the reliability repair was completed (#110, #112–#114) and the **Band 0 checkpoint was explicitly
> deferred by Taylor on 2026-09-19** while travelling without access to the Windows PC, so EXEC-02
> proceeded ahead of it. **That attended Band 0 checkpoint is not failed, not passed, not waived —
> it remains outstanding and still required.**
>
> **What EXEC-02 does and does not change here.** It creates no new stage and **moves no band**: it
> is default-off behind two explicit switches plus a required device id, it proposes rather than
> acts, and **no real-model plan-quality evidence exists** (R-EXEC01-3, open). The readiness bands
> still govern real-world testing: Band A before further unattended testing, Band B before real
> ambient sensing, Band C before full Real-World Test #2. **Listing a package or a band is not
> authorisation to start work in it**, and merging one is not authorisation to move a band.
>
