...
| **S5.3** — Executive competency reasoning | Extend `Planner.decide()` (today a stub returning `None`) so the Executive retrieves relevant competencies/knowledge/procedures for a given situation and constructs a `CandidateAction` informed by them and their confidence — through the existing Governance path, not a new one. A design is **proposed but not approved** in `docs/S5_3_EXECUTIVE_COMPETENCY_REASONING_DESIGN.md` (2026-08-11); it authorises no implementation and records five decisions (§10) needing sign-off, plus a reconnaissance finding that `decide()`'s only caller is an **ungoverned** proactive tick→bus→nudge path (§2.1). | 📋 not started |
