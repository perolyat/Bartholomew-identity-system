...
def load_policy(path: str = None) -> Dict[str, Any]:
    ...
    if _policy_cache is None:
        try:
            ...
            logger.warning(
                f"Failed to load policy.yaml: {e}, using empty policy"
            )
            _policy_cache = {}
...
def can_index(evaluated_meta: Dict[str, Any]) -> bool:
    ...
    disallow_strong = policy.get("indexing", {}).get(
    ...
    if isinstance(encrypt, str) and encrypt.lower().strip() == "strong":
        logger.info(
            "Indexing blocked by policy: encrypt=strong with "
            "disallow_strong_only enabled"
        )
        return False
