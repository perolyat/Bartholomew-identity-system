...
from bartholomew.kernel.retrieval import RetrievedItem, RetrievalFilters
...
class HybridRetrievalConfig:
    ...
    def __post_init__(self):
        ...
        if self.fusion_mode not in ("weighted", "rrf"):
            raise ValueError(
                f"fusion_mode must be 'weighted' or 'rrf', "
                f"got {self.fusion_mode}"
            )
        
        if self.normalization != "minmax":
            raise ValueError(
                f"normalization must be 'minmax', got {self.normalization}"
            )
...
class HybridRetriever:
    ...
    def __init__(
        self,
        db_path: str,
        fts: Optional[FTSClient] = None,
        vector_store: Optional[VectorStore] = None,
        embedding_engine: Optional[EmbeddingEngine] = None,
        rules_engine: Optional[MemoryRulesEngine] = None,
    ...
    def retrieve(
        ...
        weight_override: Optional[Tuple[float, float]] = None,
        api: str = "item",
        now: Optional[datetime] = None
    ) -> Union[List[RetrievedItem], List[Result]]:
        """
        ...
        if self.rules_engine:
            ...
            filtered_ids = {
                mid for mid in filtered_ids
                if rules_data.get(mid, {}).get("include", True)
            }
        ...
        boosted_fts, boosted_vec, boost_map = self._apply_boosts(
            fts_scores, vec_scores, memory_metadata, rules_data, now
        )
        ...
        if fusion_mode == "rrf":
            fused_scores = self._fuse_rrf(
                ...
                memory_metadata, rules_data, now
            )
        else:
        ...
        if self.debug_enabled:
            self._build_debug_info(
                ranked, fts_scores, vec_scores, boost_map,
                fused_scores, fusion_mode,
                call_weight_fts, call_weight_vec,
                t_fts_start, t_fts_end,
                t_vec_start, t_vec_end,
                t_fusion_start, t_fusion_end
            )
        ...
        for memory_id, score in ranked:
            ...
            snippet = self._generate_snippet(
                memory_id, metadata, query, precomputed=precomputed
            )
    ...
    def _apply_boosts(
        ...
        for memory_id in all_ids:
            ...
            recency_boost = self._compute_recency_boost(
                data.get(self.config.recency_field), now
            )
            kind_boost = self.config.kind_boosts.get(
                data.get("kind", ""), 1.0
            )
    ...
    def _fuse_weighted(
        ...
        weight_vec: Optional[float] = None
    ) -> Dict[int, float]:
        """
    ...
    def _fuse_rrf(
        ...
        for memory_id in all_ids:
            ...
            if memory_id in fts_ranks:
                rrf_contrib += (
                    1.0 / (self.config.rrf_k + fts_ranks[memory_id])
                )
            
            if memory_id in vec_ranks:
                rrf_contrib += (
                    1.0 / (self.config.rrf_k + vec_ranks[memory_id])
                )
    ...
    def _generate_snippet(
        ...
        if summary:
            excerpt = self._center_excerpt(
                summary, self.config.max_snippet_chars
            )
            return html.escape(excerpt, quote=False)
        
        value = metadata.get("value", "")
        excerpt = self._center_excerpt(
            value, self.config.max_snippet_chars
        )
        return html.escape(excerpt, quote=False)
    ...
    def _build_debug_info(
        ...
        fusion_mode: str,
        call_weight_fts: Optional[float],
        call_weight_vec: Optional[float],
        t_fts_start: float,
        t_fts_end: float,
        t_vec_start: float,
        t_vec_end: float,
        ...
        if fusion_mode == "weighted":
            w_fts = (
                call_weight_fts if call_weight_fts is not None
                else self.config.weight_fts
            )
            w_vec = (
                call_weight_vec if call_weight_vec is not None
                else self.config.weight_vec
            )
            self.last_debug["weights_used"] = {
                "fts": w_fts,
                "vec": w_vec
            }
