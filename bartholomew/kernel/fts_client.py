...
class FTSClient:
    ...
    def __init__(self, db_path: str, tokenizer: Optional[str] = None):
        """
    ...
    def migrate_schema(self) -> None:
        ...
        try:
            ...
            if has_mismatch:
                logger.warning(
                    "FTS rowid mismatch detected, rebuilding index..."
                )
    ...
    def search(
        ...
        """
        order_clause = (
            "ORDER BY rank ASC" if order_by_rank 
            else "ORDER BY m.id DESC"
        )
        ...
        try:
            ...
            if not force_fallback:
                try:
                    cursor = conn.execute(
                        sql_bm25, (query, fetch_limit, offset)
                    )
                    ...
                    if "no such function: bm25" in str(e).lower():
                        logger.info(
                            "bm25 UDF not available, using matchinfo fallback"
                        )
                        force_fallback = True
                    else:
                        raise
            ...
            if force_fallback:
                ...
                cursor = conn.execute(
                    sql_fallback, (query, fetch_limit, offset)
                )
                rows = cursor.fetchall()
                results = [dict(row) for row in rows]
        finally:
        ...
        logger.debug(
            f"FTS search returned {len(results)} results for: {query}"
        )
        return results
    ...
    def snippet(
        ...
        if column not in column_map:
            raise ValueError(
                f"Invalid column: {column}. Use 'value' or 'summary'"
            )
