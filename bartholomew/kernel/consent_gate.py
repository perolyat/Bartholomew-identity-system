...
class ConsentGate:
    ...
    def get_consented_memory_ids(self) -> Set[int]:
        ...
        try:
            ...
            cursor = conn.execute(
                "SELECT memory_id FROM memory_consent"
            )
    ...
    def filter_memory_ids(
        self,
        memory_ids: List[int],
        consented_ids: Optional[Set[int]] = None
    ) -> Dict[int, Dict[str, Any]]:
        """
        ...
        for memory_id in memory_ids:
            ...
            if not evaluated.get("allow_store", True):
                include = False
                logger.debug(
                    f"Excluding memory {memory_id}: never_store policy"
                )
            ...
            if evaluated.get("requires_consent", False):
                if memory_id not in consented_ids:
                    include = False
                    logger.debug(
                        f"Excluding memory {memory_id}: "
                        "requires_consent without consent record"
                    )
