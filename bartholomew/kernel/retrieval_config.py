...
class RetrievalConfigManager:
    ...
    def _load_config(self) -> None:
        ...
        weight_sum = (
            self._hybrid_config.weight_fts + self._hybrid_config.weight_vec
        )
        if weight_sum > 0:
    ...
    def reload(self) -> None:
        ...
        if self._hybrid_config.fusion_mode != old_fusion:
            changes.append(
                f"fusion_strategy: {old_fusion} -> "
                f"{self._hybrid_config.fusion_mode}"
            )
        ...
        if self._fts_tokenizer != old_tokenizer:
            changes.append(
                f"fts_tokenizer: {old_tokenizer} -> {self._fts_tokenizer}"
            )
        ...
        if self._fts_index_mode != old_index_mode:
            changes.append(
                f"fts_index_mode: {old_index_mode} -> {self._fts_index_mode}"
            )
        ...
        if changes:
            self._logger.info(
                f"Reloaded retrieval config: {', '.join(changes)}"
            )
        else:
    ...
    def check_and_reload_if_needed(self) -> None:
        ...
        try:
            ...
            self._logger.error(
                f"Failed to check file modification time: {e}"
            )
    ...
    def _start_watcher(self) -> None:
        ...
        self._watch_thread = threading.Thread(
            target=watch_loop, daemon=True
        )
        self._watch_thread.start()
        self._logger.debug(
            "Started background file watcher for retrieval config"
        )
...
_retrieval_config_manager: Optional[RetrievalConfigManager] = None
