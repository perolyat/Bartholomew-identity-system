    ...
    if explicit:
        ...
        if env_mode:
            ...
            try:
                ...
                logger.debug(
                    f"Failed to load mode from kernel.yaml: {e}"
                )
    ...
    def retrieve(
        ...
        for memory_id, score in sorted(
            ...
            snippet = self._generate_snippet(
                memory_id, data, query, precomputed=precomputed
            )
...
def get_retriever(
    ...
    if rules_engine is None:
        try:
            ...
            logger.warning(
                "Failed to import _rules_engine, "
                "creating new MemoryRulesEngine instance"
            )
            rules_engine = MemoryRulesEngine()
    ...
    logger.debug(
        f"Creating retriever: mode={resolved_mode}, "
