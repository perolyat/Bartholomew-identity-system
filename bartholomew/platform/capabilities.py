...
class Capability(str, Enum):
    ...
    LIVENESS = "liveness"
    METRICS = "metrics"
    # Governed inbound capture (Session D). Two capabilities, not one:
    # recording that an external event arrived and reading what has arrived
    # are different powers, and a future integration credential should be
    # able to hold the first without the second -- the same reasoning that
    # keeps MEMORY_EXPORT separate from MEMORY_READ above.
    #
    # Deliberately NOT folded into MEMORY_WRITE: capture writes no memory,
    # and a capability that conflated them would let an inbound credential
    # be mistaken for one that can alter what Bartholomew believes.
    INBOUND_CAPTURE = "inbound:capture"
