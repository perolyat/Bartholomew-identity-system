...
| Executive | Proposes a candidate action; doesn't execute it. **On chat this stage has two depths** (EXEC-02, PR #115, merged 2026-09-18 as `25cfd90`; **default-off**): every turn builds the `CandidateAction` Governance consumes, and — when explicitly enabled — an utterance recognised as an outcome-level goal is additionally handed to `bartholomew/executive/`'s goal-to-plan deliberation through the **same** seam the operator console uses. This is depth in this stage, not a second Executive; `CONSTITUTION.md`'s one-Executive rule is unaffected and `kernel/planner.py` remains inert. | `runtime_contract.py`'s `CandidateAction` dataclass; `_CHAT_DISPATCH`'s last entry `_handle_executive_goal`; `bartholomew/kernel/goal_intents.py`; `bartholomew/executive/seam.py` |
| Governance — the human's half | **A human authority decision may now arrive through conversation** (R-EXEC02-2, 2026-09-19; behind EXEC-02's same default-off activation, and no new switch). Conversation *transports* the decision; it does not become an authority. The decision is recognised deterministically from the person's own words — **never by a model** — bound to the exact proposal that was presented (action id, device, capability, version, `parameter_fingerprint`), and carried to the one approval authority, which re-reads the Parking Brake, re-validates against the device's current allowlists, enforces which capability classes the surface may carry, and consumes the approval with the same conditional UPDATE the operator console's goes through. *Cognition may propose; it never authorises* is unaffected: the Executive package still cannot mint an approval, and an AST test pins the repository-wide set of callers that can. | `bartholomew/kernel/approval_intents.py`; `bartholomew/integration/conversational_approval.py`; `_CHAT_DISPATCH`'s first entry `_handle_conversational_decision`; `bartholomew/actuation/seam.py`'s `grant_action_approval` |
...
| External capability supply (an outside model, agent, service, application or device performing a bounded task on Bartholomew's behalf) | **Skill Registry — the same owner as Capabilities, above.** An external provider is a capability, however intelligent it is internally; it is never a second decision authority, and no architectural concept may be named after a provider (added 2026-08-27, conceptual — beyond the existing model adapters, no external capability provider exists). **Corrected 2026-09-13 (FND-04):** a governed *boundary* now exists — the External Capability Interface, `bartholomew/eci/` — through which an enrolled external endpoint advertises capabilities, submits observations and requests, and carries out a directive Bartholomew issued. It does **not** change this row's ownership: the ECI transports and validates, and *which* capability is used is still decided by the seam that issues the directive, never by the boundary or by the endpoint. Advertising a capability confers no authority to decide when it is used. Still absent and still unauthorised: any capability broker, provider registry, selection or routing. | `identity_interpreter/adapters/` (local + cloud model adapters) under `ModelRouter` today; `bartholomew/eci/` for enrolled external endpoints (FND-04); remote services / MCP later |
...
| `training` | `runtime_contract.run_training_through_runtime_contract()` — fail-closed check on `training.TRAINING_BRAKE_SCOPE` **before any record in the submission is processed**, so a blocked brake yields zero writes and zero consent-queue entries |

**Added 2026-08-20:** the `training` row above was missing from this table, which listed five call
sites while the scope paragraph below correctly named six scopes. That inconsistency is resolved in
favour of six.
...
**Coverage boundary and the `sight`/`voice` migration seam (recorded 2026-08-20, Post-Test #1).**
Two facts must travel with the scope list, so that having six named scopes is not mistaken for
having six proven enforcement paths:

- Real-World Test #1's Parking Brake configuration-state matrix (64 combinations, 63 PASS / 1
  PARTIAL PASS / 0 FAIL; 128 engage/disengage transitions across revisions 16–143) directly
  validated UI selection, displayed active state, governance-state persistence, and the
  skills-backed Notifications enforcement probe. **It did not independently execute or prove direct
  `sight`, `voice`, `scheduler`, or `training` enforcement for every combination.** Safety gate S5
  in the Post-Test #1 register is where that proof belongs, and it is not discharged.
- **The `sight`/`voice` brake authority must consolidate onto the authoritative Governance path
  before real `sight`/`voice` capability is enabled** (register constraint C6; a Band B prerequisite
  in `ROADMAP.md`'s "Post-Test #1 readiness bands"). Not implemented; the 2026-08-20
  documentation-only pass deliberately did not implement it.
...
| 7 | Does every interface expose the same personality? | **Partial** — identity yes, persona no (corrected 2026-09-12 by FND-01; this cell previously read **Yes**, on the Stage 4.5 scope recorded in the note below the table, with voice/sight persona formally reclassified to Stage 6) | The Stage 4.5 goal this question encodes — *"one personality, not one personality per interface"* (MASTER_PLAN.md's P2.5 finding) — is met for every interface that produces personality-bearing output today: chat, CLI `explain`, and standalone `chat.py` all source tone from the single persona authority (`PersonaPackManager`'s active pack) after the deprecated `identity_interpreter/policies/persona.py` was removed (item 11.12, 2026-07-22). The `traits` those callers read from `Identity.yaml` are a stable identity descriptor, not persona-pack state — a deliberate split, not a convergence gap. The one remaining piece — voice/sight consulting persona — is **formally reclassified to Stage 6 (2026-07-24)**: a surface that produces no personality-bearing output cannot "expose" a personality, so converging persona onto voice/sight is inseparable from Stage 6's persona-producing voice/sight functionality (the already-approved Stage 6 boundary), not a Stage 4.5 deliverable left undone. **Corrected 2026-09-12 (FND-01):** this row's "met for chat" verdict was true of what the *callers* read, and false of what the *model* received. On the live `/api/chat` path, `PersonaPackManager`'s active pack reached the Interpretation prompt only as the identifier line `Active persona: <pack_id>`; `get_tone()` had no caller there, no system role was sent to either adapter, and no name, description, traits, core values, style guidelines or red lines reached the model at all. Only standalone `chat.py` and the CLI built a real identity-bearing system prompt. FND-01 closes the identity half: `identity_interpreter/identity_projection.py` renders canonical identity and `ModelRouter` delivers it as the provider system message on every real generation, pinned by `tests/test_fnd01_identity_projection.py`. The **persona** half of this row remains open and is deliberately out of FND-01's scope — switchable tone still does not reach the model on any surface, and this row should not be read as claiming it does. |
