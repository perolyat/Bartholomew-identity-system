...
> | **Merge Qualification** | `.github/workflows/merge-qualification.yml` | every push to a pull request; manual dispatch | The deterministic merge-readiness gate (AUDIT-CTRL-1). Runs its own regression suite, then computes whether the **exact current head** can be proven green on every required tier and has no undispositioned substantive finding. It **runs no tests of the system** and is deliberately **not** one of the tiers it requires. |
>
> **The Merge Qualification gate is not a fifth CI tier.** It proves nothing itself; it reads what the four tiers above have (or have not) proven about one commit, and refuses to report READY on anything it cannot establish. It required the **Merge Candidate** tier, which does not run on an ordinary pull request — so a pull request without the `ci:merge-candidate` label is NOT merge-qualified, which is the point. `docs/AUDIT_CTRL_1_MERGE_QUALIFICATION.md` is the authority for its semantics; `.github/merge-qualification/config.yml` is the authority for which jobs it requires.
>
