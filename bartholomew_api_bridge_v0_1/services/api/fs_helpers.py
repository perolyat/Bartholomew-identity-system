...
def wait_for_removal(path: str | Path, timeout: float = 2.0, step: float = 0.05) -> bool:
    """
...
def robust_unlink(path: str | Path, retries: int = 10, step: float = 0.05) -> None:
    """
...
def robust_rmtree(path: str | Path, retries: int = 10, step: float = 0.1) -> None:
    """
...
def wal_aux_paths(db_path: str | Path) -> tuple[Path, Path]:
    """
